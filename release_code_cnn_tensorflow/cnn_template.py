...
def sgd(model, X_train, y_train, minibatch_size):
    for iter in range(n_iter):
        ...
        print("sgd by minibatch")
        for i in range(0, X_size[0], minibatch_size):
...
if __name__ == "__main__":
	...
	for k in range(n_experiment):
	    exp = [str(k) + 'th experimence']
	    print(exp)
	    # Reset model
	    model = make_network()

	    print("training model by stochastic gradient descent") # Train the model
